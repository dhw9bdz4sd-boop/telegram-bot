...
def smart_signal(pair):
    ...
    # إذا خسائر كثيرة → نوقف
    if loss_streak >= 3:
        return None

    # فلترة السوق (منع التذبذب)
    if 45 < r < 55:
        return None

    # BUY
    if r < 45 and ema_fast > ema_slow:
        return "BUY ⬆️", 80

    # SELL
    if r > 55 and ema_fast < ema_slow:
        return "SELL ⬇️", 80
...
def auto_signal_loop():
    ...
    while True:
        for chat_id, user in users.items():
            ...
            # 🔔 تنبيه قبل الصفقة
            send_message(chat_id,"⏳ بعد 60 ثانية صفقة قادمة...")
